...
def test_short_titles():
    headers = [["title1", "title1", "title2", "title2"], ["subtitle1", "subtitle2", "subtitle1", "subtitle2"]]
...
def test_long_titles():
    headers = [["somereallyreallylongtitle1", "somereallyreallylongtitle1", "somereallyreallylongtitle2", "somereallyreallylongtitle2"], ["subtitle1", "subtitle2", "subtitle1", "subtitle2"]]
    df = pl.DataFrame({"a": [1, 1, 2], "b": [3, 4, 5], "c": 6, "d": 7})
    result = "\n" + cerbursus.to_string(df, headers)
    expected = """
...
def test_three_levels_long_values():
    headers = [["alpha", "alpha", "beta", "beta"], ["a", "a", "b", "b"], ["c", "d", "c", "d"]]
    df = pl.DataFrame({"a": [10000000, 1, 2], "b": [3, 4, 5], "c": 10000000, "d": 7})
    result = "\n" + cerbursus.to_string(df, headers)
    expected = """
    alpha          beta    
